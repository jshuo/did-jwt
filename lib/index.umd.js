(function (global, factory) {
  ...
  (global = global || self, factory(global.didJwt = {}, global.uint8Arrays, global.ed25519, global.multiformats, global.multibase, global.secp256k1$1, global.p256, global.sha256$2, global.ripemd160$1, global.sha3, global.canonicalize, global.didResolver, global.base$1, global.chacha, global.utils$1));
}(this, (function (exports, uint8arrays, ed25519, multiformats, multibase, secp256k1$1, p256, sha256$2, ripemd160$1, sha3, canonicalizeData, didResolver, base$1, chacha, utils$1) {
  ...
  function extractPublicKeyBytes(pk) {
    if (pk.publicKeyBase58) {
      ...
      return {
        keyBytes: secp256k1$1.secp256k1.ProjectivePoint.fromAffine({
  ...
  function multibaseToBytes(s) {
    const bytes = multibase.decode(s);
  ...
  const lengthAndInput = input => uint8arrays.concat([writeUint32BE(input.length), input]);
  ...
  function concatKDF(secret, keyLen, alg, producerInfo, consumerInfo) {
    ...
    const value = uint8arrays.concat([lengthAndInput(uint8arrays.fromString(alg)), lengthAndInput(typeof producerInfo === 'undefined' ? new Uint8Array(0) : producerInfo), lengthAndInput(typeof consumerInfo === 'undefined' ? new Uint8Array(0) : consumerInfo), writeUint32BE(keyLen)]);
    // since our key lenght is 256 we only have to do one round
    const roundNumber = 1;
    return sha256$1(uint8arrays.concat([writeUint32BE(roundNumber), secret, value]));
  }
  ...
  function ES256KSigner(privateKey, recoverable = false) {
    ...
    return function (data) {
      try {
        const signature = secp256k1$1.secp256k1.sign(sha256$1(data), privateKeyBytes);
  ...
  function ES256Signer(privateKey) {
    ...
    return function (data) {
      try {
        const signature = p256.p256.sign(sha256$1(data), privateKey);
        return Promise.resolve(toJose({
          r: leftpad(signature.r.toString(16)),
          s: leftpad(signature.s.toString(16))
        }));
  ...
  function publicKeyToAddress$1(publicKey, otherAddress) {
    ...
    const publicKeyHash = ripemd160$1.ripemd160(sha256$1(publicKeyBuffer));
    const step1 = version + bytesToHex(publicKeyHash);
    const step2 = sha256$1(hexToBytes(step1));
    const step3 = sha256$1(step2);
  ...
  function publicKeyToAddress(publicKey, prefix) {
    const publicKeyBuffer = secp256k1$1.secp256k1.ProjectivePoint.fromHex(publicKey).toRawBytes();
    const hash = ripemd160$1.ripemd160(sha256$1(publicKeyBuffer));
    const words = base$1.bech32.toWords(hash);
    return base$1.bech32.encode(prefix, words).replace(prefix, '');
  }
  ...
  (function (module, exports) {
    ...
    try {
      if (typeof window !== 'undefined' && typeof window.Buffer !== 'undefined') {
        ...
        Buffer = _nodeResolve_empty$1.Buffer;
      }
    } catch (e) {
    }
  ...
  if (typeof self === 'object') {
    ...
    try {
      var crypto = _nodeResolve_empty$1;
  ...
  */
  ...
  function verifyES256K(data, signature, authenticators) {
    const hash = sha256$1(data);
    const signatureNormalized = secp256k1$1.secp256k1.Signature.fromCompact(base64ToBytes(signature)).normalizeS();
    ...
    let signer = fullPublicKeys.find(pk => {
      try {
        ...
        return secp256k1$1.secp256k1.verify(signatureNormalized, hash, keyBytes);
      } catch (err) {
        return false;
      }
    });
  ...
  function verifyRecoverableES256K(data, signature, authenticators) {
    ...
    const checkSignatureAgainstSigner = sigObj => {
      const signature = secp256k1$1.secp256k1.Signature.fromCompact(sigObj.compact).addRecoveryBit(sigObj.recovery || 0);
  ...
  function xc20pEncrypter(key) {
    return (cleartext, aad) => {
      const iv = utils$1.randomBytes(24);
  ...
  const createX25519EcdhEsKek = function (recipientPublicKey, senderSecret,
  // unused
  alg, apu,
  // unused
  apv, ephemeralKeyPair) {
    try {
      ...
      const sharedSecret = ed25519.x25519.getSharedSecret(ephemeral.secretKey, recipientPublicKey);
      // Key Encryption Key
  ...
  const createX25519Ecdh1PUv3Kek = function (recipientPublicKey, senderSecret, alg,
  // must be provided as this is the key agreement alg + the key wrapper alg, Example: 'ECDH-ES+A256KW'
  apu, apv, ephemeralKeyPair) {
    try {
      ...
      const zE = ed25519.x25519.getSharedSecret(ephemeral.secretKey, recipientPublicKey);
  ...
  function createFullEncrypter(recipientPublicKey, senderSecret, options = {}, kekCreator, keyWrapper, contentEncrypter) {
    const encrypt = function (cleartext, protectedHeader = {}, aad, ephemeralKeyPair) {
      try {
        ...
        const cek = utils$1.randomBytes(32);
