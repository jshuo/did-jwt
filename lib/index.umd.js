(function (global, factory) {
  ...
  function verifyES256(data, signature, authenticators) {
    ...
    const checkSignatureAgainstSigner = sigObj => {
      const signature = p256.p256.Signature.fromCompact(sigObj.compact).addRecoveryBit(sigObj.recovery || 0);
  ...
  function verifyRecoverableES256K(data, signature, authenticators, ES256) {
    const signatures = [];
    ...
    const checkSignatureAgainstSigner = sigObj => {
      ...
      if (ES256 === "ES256") {
        signature = p256.secp256r1.Signature.fromCompact(sigObj.compact).addRecoveryBit(sigObj.recovery || 0);
      } // {  signature = p256.Signature.fromCompact(sigObj.compact).addRecoveryBit(sigObj.recovery || 0)} 
      else if (ES256 = "ES256K-R") {
        {
          signature = secp256k1.secp256k1.Signature.fromCompact(sigObj.compact).addRecoveryBit(sigObj.recovery || 0);
        }
      }
  ...
  function verifyJWSDecoded({
    ...
    if (!Array.isArray(pubKeys)) pubKeys = [pubKeys];
    const signer = VerifierAlgorithm(header.alg)(data, signature, pubKeys, header.alg);
    return signer;
  }
  /**
