...
import { secp256k1 as secp256k1$1 } from '@noble/curves/secp256k1';
import { p256 } from '@noble/curves/p256';
import { sha256 as sha256$2 } from '@noble/hashes/sha256';
import { ripemd160 as ripemd160$1 } from '@noble/hashes/ripemd160';
...
function concatKDF(secret, keyLen, alg, producerInfo, consumerInfo) {
  ...
  const value = concat([lengthAndInput(fromString(alg)), lengthAndInput(typeof producerInfo === 'undefined' ? new Uint8Array(0) : producerInfo), lengthAndInput(typeof consumerInfo === 'undefined' ? new Uint8Array(0) : consumerInfo), writeUint32BE(keyLen)]);
  // since our key lenght is 256 we only have to do one round
  const roundNumber = 1;
  return sha256$1(concat([writeUint32BE(roundNumber), secret, value]));
}
...
function ES256KSigner(privateKey, recoverable = false) {
  ...
  return function (data) {
    try {
      const signature = secp256k1$1.sign(sha256$1(data), privateKeyBytes);
...
function ES256Signer(privateKey) {
  ...
  return function (data) {
    try {
      const signature = p256.sign(sha256$1(data), privateKey);
      return Promise.resolve(toJose({
        r: leftpad(signature.r.toString(16)),
        s: leftpad(signature.s.toString(16))
      }));
...
function publicKeyToAddress$1(publicKey, otherAddress) {
  ...
  const publicKeyHash = ripemd160$1(sha256$1(publicKeyBuffer));
...
function publicKeyToAddress(publicKey, prefix) {
  ...
  const hash = ripemd160$1(sha256$1(publicKeyBuffer));
